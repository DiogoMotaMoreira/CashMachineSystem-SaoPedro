...
class AppCozinha(ctk.CTk):
    ...
    def criar_card_pedido(self, texto_pedido):
        ...
        partes = texto_pedido.split(maxsplit=1)
        ...
        lbl_num = ctk.CTkLabel(card, text=numero, font=("Monospace", 26, "bold"), 
                              text_color="#f9e2af", width=120)
        lbl_num.pack(side="left", padx=15, pady=15)

        # Itens do Pedido (Destaque Direito)
        lbl_itens = ctk.CTkLabel(card, text=itens, font=("Arial", 22, "bold"), 
                                text_color="#cdd6f4", wraplength=800, justify="left")
        lbl_itens.pack(side="left", padx=15, fill="x")
        ...
        if os.path.exists("pedidos.txt"):
            try:
                with open("pedidos.txt", "r", encoding="utf-8") as f:
                    linhas = f.readlines()
                    # Aumentado para os últimos 15 pedidos já que o texto é menor
                    pedidos_atuais = [l.strip() for l in linhas[-15:] if l.strip()]
            except Exception:
                pass
                
        conteudo_atual = str(pedidos_atuais)
        # Só redesenha se houver novos pedidos ou se o ficheiro for limpo/apagado
        if conteudo_atual != self.last_content:
            ...
            self.last_content = conteudo_atual
